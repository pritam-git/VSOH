...
class Mobi_Mtld_DA_Api
{
	...
	private static function _getProperties(array &$tree, $userAgent, $typedValues)
	{
		...
		$matched = "";
		$sought = NULL;
		self::_seekProperties($tree['t'], trim($userAgent), $idProperties, $sought, $matched);
		$properties = array();
	 ...
	 * @access private
	 */
	private static function _seekProperties(array &$node, $string, array &$properties, &$sought, &$matched)
	{
		$unmatched = $string;
		...
		if(isset($node['c'])) {
			for($c = 1; $c < strlen($string) + 1; $c++) {
				...
				if(isset($node['c'][$seek])) {
					$matched .= $seek;
					self::_seekProperties($node['c'][$seek], substr($string, $c), $properties, $sought, $matched);
					break;
				}
			}
		}
