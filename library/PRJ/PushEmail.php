 ...
 *
 *
 * @filesource /library/PRJ/PushEmail.php
 * @author     Krishna Bhatt <krishna.patel@bcssarl.com>
 * @version    $Id: PushEmail.php 16 2019-03-07 17:27:56Z sl $
 */
...
class PRJ_PushEmail
{
	 ...
	 * @return array
	 */
	public static function processPushEmail($M2nModel,$protocol,$requestParams,$controller,$short,$regionId = NULL) {
		//view from MVC
		$viewFromMVC = Zend_Layout::getMvcInstance()->getView();
...
//			->limit(L8M_Config::getOption('l8m.news.maxPerPage'))
		...
		if ($resultCount > 0) {
			...
			foreach ($resultCollection as $entityModel) {
				...
				$redirectTo = L8M_Library::getSchemeAndHttpHost() . $viewFromMVC->url(array('module' => 'default', 'controller' => $controller, 'action' => 'detail', 'lang'=> strtolower($entityModel->spoken_language), 'short' => $short), NULL, TRUE);

				if(!empty($regionId))
					$redirectTo = L8M_Library::getSchemeAndHttpHost() . $viewFromMVC->url(array('module' => 'default', 'controller' => $controller, 'action' => 'detail', 'lang'=> strtolower($entityModel->spoken_language), 'region'=>$regionId, 'short' => $short), NULL, TRUE);

				//send emails to user
				PRJ_Email::send('push_email', $entityModel, array('REDIRECT_URL' => $redirectTo));
			}
		}
	...
	public static function sendTestMail($requestParams,$controller,$short,$regionId = NULL){
		...
		foreach ($requestParams['selectedLang'] as $lang) {
			//redirect link for protocol page
			$redirectTo = L8M_Library::getSchemeAndHttpHost() . $viewFromMVC->url(array('module' => 'default', 'controller' => $controller, 'action' => 'detail', 'lang'=> strtolower($lang), 'short' => $short), NULL, TRUE);

			if(!empty($regionId))
				$redirectTo = L8M_Library::getSchemeAndHttpHost() . $viewFromMVC->url(array('module' => 'default', 'controller' => $controller, 'action' => 'detail', 'lang'=> strtolower($lang), 'region'=>$regionId, 'short' => $short), NULL, TRUE);
			...
			//send emails to user
			PRJ_Email::send('push_email', (object)$entityModel, array('REDIRECT_URL' => $redirectTo));
		}
