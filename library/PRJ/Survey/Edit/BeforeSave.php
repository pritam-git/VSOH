 ...
 *
 *
 * @filesource /library/PRJ/Survey/Edit/BeforeSave.php
 * @author	   Unnati Visani <unnati.visani@bcssarl.com>
 * @version    $Id: BeforeSave.php 7 2019-11-01 14:20:40Z nm $
 */
 ...
 *
 */
class PRJ_Survey_Edit_BeforeSave
{
	...
	public function beforeSave($modelID, $modelName, $formValues)
	{
		...
		if ($model !== FALSE) {
			...
			if(isset($this->_formValues['survey_data'])) {
				...
				$checkIfSerialized = @unserialize($this->_formValues['survey_data']);

				if($checkIfSerialized === FALSE) {
