 ...
 * @version    $Id: Image.php 435 2015-09-28 15:14:38Z nm $
 */

/** @see Zend_Captcha_Word */
require_once 'Zend' . DIRECTORY_SEPARATOR . 'Captcha' . DIRECTORY_SEPARATOR . 'Word.php';
...
class Zend_Captcha_Image extends Zend_Captcha_Word
{
	...
	/**
	 * array of color for text
	 *
	 * @var array
	 */
	protected $_textColor = array(
		'red'=>0,
		'green'=>0,
		'blue'=>0,
	);
	...
	/**
	 * flags whether color for text is grey or not
	 *
	 * @var boolean
	 */
	protected $_textColorIsGrey = TRUE;

	/**
	 * array of color for background
	 *
	 * @var array
	 */
	...
	public function setTextColor ($textColor)
	{
		if (is_array($textColor) &&
			array_key_exists('red', $textColor) &&
			array_key_exists('green', $textColor) &&
			array_key_exists('blue', $textColor) &&
			$textColor['red'] >= 0 &&
			$textColor['red'] <= 255 &&
			$textColor['green'] >= 0 &&
			$textColor['green'] <= 255 &&
			...
			throw new L8M_Exception('TextColor needs to be specified as color-array.');
		}

		if ($textColor['red'] == $textColor['green'] &&
			$textColor['green'] == $textColor['blue']) {

			$this->_textColorIsGrey = TRUE;
		} else {
			$this->_textColorIsGrey = FALSE;
		}
		return $this;
	}
	...
	public function setBackgroundColor ($backgroundColor)
	{
		if (is_array($backgroundColor) &&
			array_key_exists('red', $backgroundColor) &&
			array_key_exists('green', $backgroundColor) &&
			array_key_exists('blue', $backgroundColor) &&
			...
			throw new L8M_Exception('BackgroundColor needs to be specified as color-array.');
		}
		return $this;
	}
	...
	protected function _generateImage($id, $word)
	{
		if (!extension_loaded("gd")) {
			require_once 'Zend' . DIRECTORY_SEPARATOR . 'Captcha' . DIRECTORY_SEPARATOR . 'Exception.php';
		...
		$text_color = imagecolorallocate($img, $this->_textColor['red'], $this->_textColor['green'], $this->_textColor['blue']);
		$bg_color   = imagecolorallocate($img, $this->_backgroundColor['red'], $this->_backgroundColor['green'], $this->_backgroundColor['blue']);
		...
		$bg_color = imagecolorallocate($img2, $this->_backgroundColor['red'], $this->_backgroundColor['green'], $this->_backgroundColor['blue']);
		...
		for ($x = 0; $x < $w; $x++) {
			for ($y = 0; $y < $h; $y++) {
				...
				if ($color == $this->_backgroundColor['red'] && $color_x == $this->_backgroundColor['red'] && $color_y == $this->_backgroundColor['red'] && $color_xy == $this->_backgroundColor['red']) {
					// ignore background
					continue;
				} elseif ($color == $this->_textColor['red'] && $color_x == $this->_textColor['red'] && $color_y == $this->_textColor['red'] && $color_xy == $this->_textColor['red']) {
					// transfer inside of the image as-is
					$newcolorRed = $this->_textColor['red'];
					$newcolorGreen = $this->_textColor['green'];
					$newcolorBlue = $this->_textColor['blue'];
				} else {
					...
					$newcolorRed = $color	* $frac_x1 * $frac_y1
							  + $color_x  * $frac_x  * $frac_y1
							  + $color_y  * $frac_x1 * $frac_y
							  + $color_xy * $frac_x  * $frac_y;

					if (!$this->_textColorIsGrey) {
						if ($color == $this->_textColor['red']) {
							$colorTG = $this->_textColor['green'];
						} else {
							$colorTG = $this->_backgroundColor['green'];
						}
						if ($color_x == $this->_textColor['red']) {
							$color_xTG = $this->_textColor['green'];
						} else {
							$color_xTG = $this->_backgroundColor['green'];
						}
						if ($color_y == $this->_textColor['red']) {
							$color_yTG = $this->_textColor['green'];
						} else {
							$color_yTG = $this->_backgroundColor['green'];
						}
						if ($color_xy == $this->_textColor['red']) {
							$color_xyTG = $this->_textColor['green'];
						} else {
							$color_xyTG = $this->_backgroundColor['green'];
						}
						$newcolorGreen = $colorTG	* $frac_x1 * $frac_y1
								  + $color_xTG  * $frac_x  * $frac_y1
								  + $color_yTG  * $frac_x1 * $frac_y
								  + $color_xyTG * $frac_x  * $frac_y;
					} else {
						$newcolorGreen = $color	* $frac_x1 * $frac_y1
								  + $color_x  * $frac_x  * $frac_y1
								  + $color_y  * $frac_x1 * $frac_y
								  + $color_xy * $frac_x  * $frac_y;
					}
					...
					if (!$this->_textColorIsGrey) {
						if ($color == $this->_textColor['red']) {
							$colorTB = $this->_textColor['blue'];
						} else {
							$colorTB = $this->_backgroundColor['blue'];
						}
						if ($color_x == $this->_textColor['red']) {
							$color_xTB = $this->_textColor['blue'];
						} else {
							$color_xTB = $this->_backgroundColor['blue'];
						}
						if ($color_y == $this->_textColor['red']) {
							$color_yTB = $this->_textColor['blue'];
						} else {
							$color_yTB = $this->_backgroundColor['blue'];
						}
						if ($color_xy == $this->_textColor['red']) {
							$color_xyTB = $this->_textColor['blue'];
						} else {
							$color_xyTB = $this->_backgroundColor['blue'];
						}
						$newcolorBlue = $colorTB	* $frac_x1 * $frac_y1
								  + $color_xTB  * $frac_x  * $frac_y1
								  + $color_yTB  * $frac_x1 * $frac_y
								  + $color_xyTB * $frac_x  * $frac_y;
					} else {
						$newcolorBlue = $color	* $frac_x1 * $frac_y1
								  + $color_x  * $frac_x  * $frac_y1
								  + $color_y  * $frac_x1 * $frac_y
								  + $color_xy * $frac_x  * $frac_y;
					}
				}
				imagesetpixel($img2, $x, $y, imagecolorallocate($img2, $newcolorRed, $newcolorGreen, $newcolorBlue));
			}
		}
