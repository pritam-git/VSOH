 ...
 * @version    $Id: Hostname.php 570 2018-08-03 08:42:35Z nm $
 */
 ...
 */
require_once 'Zend' . DIRECTORY_SEPARATOR . 'Validate' . DIRECTORY_SEPARATOR . 'Ip.php';
...
class Zend_Validate_Hostname extends Zend_Validate_Abstract
{
    ...
    public function isValid($value)
    {
        ...
        if ((count($domainParts) > 1) && (strlen($value) >= 4) && (strlen($value) <= 254)) {
                ...
                if (preg_match('/([^.]{2,10})$/i', end($domainParts), $matches) ||
                    ...
                    if ($this->_options['idn'] &&  isset($this->_validIdns[strtoupper($this->_tld)])) {
                        if (is_string($this->_validIdns[strtoupper($this->_tld)])) {
                            $regexChars += include(str_replace('/', DIRECTORY_SEPARATOR, $this->_validIdns[strtoupper($this->_tld)]));
                        } else {
            ...
            (PHP_VERSION_ID < 50600) ? iconv_set_encoding('internal_encoding', $origenc) : ini_set('default_charset', $origenc);
