 ...
 *
 *
 * @filesource /library/L8M/JQuery/Form/Element/Parent.php
 ...
 *
 *
 * L8M_JQuery_Form_Element_Parent
 *
 *
 */
class L8M_JQuery_Form_Element_Parent extends Zend_Form_Element_Xhtml
{
	...
	public function __construct($spec = NULL, $localClassName = NULL, $one2NmultiRelationCondition = NULL, $one2NreplaceColumnValueInMultiRelation = NULL, $values = NULL, $editID = NULL, $rowNameKey = NULL, $idColumn = NULL, $options = NULL)
	{
		...
		if (!class_exists($localClassName)) {
			throw new L8M_Exception('Failure creationg Parent-Form-Element. Class does not exist.');
		} else {
		...
		if (!is_array($one2NmultiRelationCondition)) {
			throw new L8M_Exception('Failure creationg Parent-Form-Element. Conditions have to be set in an array.');
		} else {
		 ...
		 */
		if (!is_array($values)) {
			throw new L8M_Exception('Failure creationg Parent-Form-Element. Vales have to be set in an array.');
		} else {
		...
		if ($idColumn &&
			...
			throw new L8M_Exception('Failure creationg Parent-Form-Element. ID-Column does not exist.');
		}
