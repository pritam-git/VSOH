 ...
 *
 *
 * @filesource /library/L8M/Model/Position/Existing.php
 * @author     Norbert Marks <nm@l8m.com>
 * @version    $Id: Existing.php 42 2014-04-02 14:29:27Z nm $
 */

/**
 *
 *
 * L8M_Model_Position_Existing
 *
 *
 */
class L8M_Model_Position_Existing
{
	...
	public function __construct($model, $parentRelation) {
		if ($model &&
			...
			$this->_modelID = $this->_model->id;
			$this->_modelShort = $this->_model->short;
	...
	private function _initPosition($offset = NULL) {
		 ...
		 * id has same parent
		 */
		$hasParentSqlQuery = L8M_Sql::factory($this->_modelName)
			->addWhere('id = ?', array($this->_modelID))
		;
		if ($this->_parentRelation) {
			if ($this->_parentID) {
				$hasParentSqlQuery = $hasParentSqlQuery
		...
		$this->_notInParent = TRUE;
		if ($hasParentSqlQuery->getCount()) {
			$this->_notInParent = FALSE;
		}
		 ...
		 * own position
		 */
		$positionSqlQuery = L8M_Sql::factory($this->_modelName);

		if ($this->_parentRelation) {
			if ($this->_parentID) {
				$positionSqlQuery = $positionSqlQuery
					->addWhere($this->_parentRelation->getLocalColumnName() . ' = ?', array($this->_parentID))
				;
			} else {
				$positionSqlQuery = $positionSqlQuery
		...
		$this->_itemPosition = $positionSqlQuery
			->addWhere('position <= ?', array($this->_model->position))
			->getCount()
		;
		...
		if ($this->_notInParent) {
			...
			if (count($localCollectionArray) == 1 &&
				...
				if (count($checkCollectionArray) == 1 &&
					...
					$localCollectionArray[0]['position'] = $checkCollectionArray[0]['position'];
				}
			}
		}

		if (!isset($localCollectionArray[0]['position'])) {
