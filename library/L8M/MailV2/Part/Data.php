 ...
 *
 *
 * L8M_MailV2_Part_Data
 *
 *
 */
class L8M_MailV2_Part_Data extends L8M_MailV2_Part
{
	...
	public function setData($data = NULL)
	{
		if ($data &&
			...
			throw new L8M_MailV2_Part_Data_Exception('Data needs to be passed as an array.');
		}
		$this->_data = $data;
		return $this;
	}
	...
	/**
	 * Adds data to L8M_MailV2_Part_Data instance.
	 *
	 * @param  string $label
	 * @param  string $value
	 * @return L8M_MailV2_Part_Data
	...
	public function addData($label = NULL, $value = NULL)
	{
		if (!$label ||
			!is_string($label)) {
			throw new L8M_MailV2_Part_Data_Exception('Label needs to be specified as string.');
		}
			...
			if ($mode == self::RENDER_TEXT) {
				...
				foreach($data as $item) {
					...
					if (array_key_exists('label', $item) &&
						...
						$content = $indent
								 . str_pad($this->utf8Decode($item['label']), $maxLabelLength, ' ', STR_PAD_RIGHT)
								 . ' : '
								 . $this->utf8Decode($item['data'])
								 . PHP_EOL
						;
						echo $content;
...
?>
<table style="<?php echo $this->getParent()->getHtmlCssStyle('html_data_css_style'); ?>">
<?php
...
?>
	<tr>
		<td style="<?php echo $this->getParent()->getHtmlCssStyle('html_dataline_label_css_style'); ?>">
			<?php echo $this->escape($this->utf8Decode($item['label'])); ?>
		</td>
		<td style="<?php echo $this->getParent()->getHtmlCssStyle('html_dataline_data_css_style'); ?>">
			<?php echo $this->escape($this->utf8Decode($item['data'])); ?>
		</td>
	</tr>
<?php
...
<?php
	...
	/**
	 * Returns an L8M_MailV2_Part_Data instance from the provided form.
	 *
	...
	public static function fromForm($form = NULL, $options = array(), $isUtf8 = TRUE, $formMatchingValues = NULL)
	{
		if (!($form instanceof Zend_Form)) {
			throw new L8M_MailV2_Part_Data_Exception('Form needs to be specified as Zend_Form instance.');
		}
		...
		$partData = new L8M_MailV2_Part_Data();
	...
	{
		$partData = new L8M_MailV2_Part_Data($options);
		$partData->setEmailTemplatePartShort($emailTemplateShort);
