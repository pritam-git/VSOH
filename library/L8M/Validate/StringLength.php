 ...
 */
class L8M_Validate_StringLength extends Zend_Validate_Abstract
{
	...
	public function __construct($options = array())
	{
		...
		$this->_messageTemplates[self::TOO_SHORT] = $view->translate("'%value%' is less than %min% characters long");
		$this->_messageTemplates[self::TOO_LONG] = $view->translate("'%value%' is more than %max% characters long");
	}
	...
	public function setMin($min)
	{
		if (null !== $this->_max && $min > $this->_max) {
			 ...
			 */
			require_once 'Zend' . DIRECTORY_SEPARATOR . 'Validate' . DIRECTORY_SEPARATOR . 'Exception.php';
	...
	public function setEncoding($encoding = null)
	{
		if ($encoding !== null) {
			...
			(PHP_VERSION_ID < 50600) ? $result = iconv_set_encoding('internal_encoding', $encoding) : $result = ini_set('default_charset', $encoding);
			if (!$result) {
				(PHP_VERSION_ID < 50600) ? iconv_set_encoding('internal_encoding', $orig) : ini_set('default_charset', $orig);
				require_once 'Zend' . DIRECTORY_SEPARATOR . 'Validate' . DIRECTORY_SEPARATOR . 'Exception.php';
