...
class Doctrine_Template_Temporal extends Doctrine_Template {
    ...
    public function getCurrentSegment($date = null) {
        ...
        foreach ($record->Segments as $s) {
            // if it's currently active, then we've found the best result we can hope for
    ...
    private function shiftOverlappingRecord(Doctrine_Record &$existing_record) {
        ...
        $s_eff = $saving_record->getEffectiveDate();
        $e_eff = $existing_record->getEffectiveDate();
        $s_exp = $saving_record->getExpirationDate() ? $saving_record->getExpirationDate() : 'null';
        $e_exp = $existing_record->getExpirationDate() ? $existing_record->getExpirationDate() : 'null';
