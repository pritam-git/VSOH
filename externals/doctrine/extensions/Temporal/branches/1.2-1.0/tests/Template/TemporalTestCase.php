...
class Doctrine_Template_Temporal_TestCase extends Doctrine_UnitTestCase
{
    ...
    public function testCreateNewTemporalSegment() {
        ...
        $this->assertTrue($record2->exists());
        $this->assertTrue($record->containsDate($this->_dates[0]));
        $this->assertTrue($record2->containsDate($this->_dates[1]));
        $this->assertFalse($record->containsDate($this->_dates[1]));
        $this->assertFalse($record2->containsDate($this->_dates[0]));

        // future
        $record = $this->getParentRecord('omegaParent', $this->_dates[1]);
        $this->assertTrue($record->exists());
        $record2 = $record->createNewTemporalSegment($this->_dates[2]);
        $this->assertTrue($record2->exists());
        $this->assertTrue($record->containsDate($this->_dates[1]));
        $this->assertTrue($record2->containsDate($this->_dates[2]));
        $this->assertFalse($record->containsDate($this->_dates[2]));
        $this->assertFalse($record2->containsDate($this->_dates[1]));
    ...
    public function testGetCurrentSegment() {
        ...
        $this->assertEqual($segment->name, 'pastSegment-1'); // the last-past segment

        $parent = $this->getParentRecord('currentParent');
        $segment = $parent->getCurrentSegment();
        $this->assertEqual($segment->name, 'currentSegment0'); // the segment that's actually current

        $parent = $this->getParentRecord('futureParent', $this->_dates[1]);
        $segment = $parent->getCurrentSegment();
        $this->assertEqual($segment->name, 'futureSegment1'); // the first-future segment
    }

    public function testGetLastSegment() {
        $parent = $this->getParentRecord('pastParent', $this->_dates[-1]);
        $segment = $parent->getLastSegment();
        $this->assertEqual($segment->name, 'pastSegment-1');

        $parent = $this->getParentRecord('currentParent');
        $segment = $parent->getLastSegment();
        $this->assertEqual($segment->name, 'currentSegment1');

        $parent = $this->getParentRecord('futureParent', $this->_dates[1]);
        $segment = $parent->getLastSegment();
        $this->assertEqual($segment->name, 'futureSegment2');
    }
    ...
    public function testGetNextGetPrevious() {
        ...
        $this->assertEqual($next->name, 'currentSegment1');
        $previous = $segment->getPrevious();
        $this->assertEqual($previous->name, 'currentSegment-1');
    }
    ...
    public function testChronologicalRelations() {
        ...
        $segments = Doctrine_Query::create()
            ...
            ->where('name LIKE ?', 'currentSegment%')
            ->execute();
        $this->assertEqual($segments[0]->name, 'currentSegment0');

        // now traverse the relation to make sure the order is fixed during relation travern
        $parent = $this->getParentRecord('currentParent');
        $segments = $parent->Segments;
        $this->assertEqual($segments[0]->name, 'currentSegment-1'); // relation should have been ordered chronological
    }
    ...
    private function reloadAll() {
        ...
        $segment = new TemporalTestParentSegment();
        $segment->name = 'currentSegment1';
        $segment->eff_date = $this->_dates[1];
        $segment->exp_date = $this->_dates[2];
        $parent->Segments[] = $segment;
        $segment->save();

        $segment = new TemporalTestParentSegment();
        $segment->name = 'currentSegment-1'; // this is out of order so that we can test chronological relation query
        $segment->eff_date = $this->_dates[-1];
        ...
        $child = new TemporalTestParentSegment();
        $child->name = 'futureSegment1';
        $child->eff_date = $this->_dates[1];
        $child->exp_date = $this->_dates[2];
        $parent->Segments[] = $child;
        $child = new TemporalTestParentSegment();
        $child->name = 'futureSegment2';
        $child->eff_date = $this->_dates[2];
...
class TemporalTestParent extends Doctrine_Record {
    ...
    public function setUp() {
        $this->hasMany('TemporalTestParentSegment as Segments', array(
            'local'     => 'id',
            'foreign'   => 'parent_id',
            'orderBy'   => 'eff_date',
        ));
        ...
        $temporal0 = new Doctrine_Template_Temporal(array(
            'children'                  => array('Segments', 'Children'),
...
class TemporalTestParentSegment extends Doctrine_Record {
    public function setTableDefinition() {
        $this->hasColumn('id', 'integer', 10, array(
             'type' => 'integer',
             'unsigned' => true,
