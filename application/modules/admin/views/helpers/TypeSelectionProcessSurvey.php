 ...
 *
 */
class Admin_View_Helper_TypeSelectionProcessSurvey extends Zend_View_Helper_Abstract
{
	 ...
	 * @return string
	 */
	public function typeSelectionProcessSurvey($controller)
	{
		ob_start();
		...
		if($this->view->sent == TRUE) {
		?>
			<script>
				var redirectUrl = '<?php echo $this->view->redirectUrl; ?>';
				$('#content').append('<div id="successBlock"><h3><?php echo $this->view->translate('Survey Already Sent.'); ?></h3></div>');
		...
		<?php
			...
			if($this->view->isTested) {
				if($this->view->language &&
					...
					<script>
						var redirectUrl = '<?php echo $this->view->redirectUrl; ?>';
						var id = '<?php echo $this->view->surveyId; ?>';
					...
					$('#sendTestMailBtn').click(function(){
						...
						if (isValid === true){
							...
							$.ajax({
								...
								url: ajaxUrl,
								data: {
									id: '<?php echo $this->view->surveyId; ?>',
								...
								success: function(response){
									if(response.isSent == true){
										...
										if($('#testDoneWell').length <= 0){
											$('#testEmailBlock').append('<a href="<?php echo $this->view->currentUrl.'id='.$this->view->surveyId.'&do=TRUE'; ?>" class="btn btn-success" id="testDoneWell"><?php echo $this->view->translate('Test went well'); ?></a>');
										}
									}
								}
							}).fail(function(response){
