 ...
 *
 *
 * @filesource /application/modules/admin/controllers/OtcController.php
 * @author     Krishna Bhatt <krishna.patel@bcssarl.com>
 * @version    $Id: OtcController.php 299 2019-01-08 10:15:38Z nm $
 */
 ...
 *
 */
class Admin_OtcController extends L8M_Controller_Action
{
	/**
	 *
	 *
	 ...
	 *
	 */
	private $_modelListName = 'Default_Model_Otc';
	private $_modelListShort = 'Otc';
	private $_modelListConfig = array();
	private $_modelListUntranslatedTitle = 'Otc';
	...
	/**
	 * Initializes Admin_OtcController.
	 *
	 * @return void
	 */
...
//			->setButton('Update', array('action'=>'update', 'controller'=>'action', 'module'=>'system'), 'update', FALSE)
			->setButton('Send PushMail', array('action'=>'update', 'controller'=>'otc', 'module'=>'admin'), 'update', TRUE)
//			->disableSaveWhere()
//			->useDbWhere(FALSE)
//			->showAjax();
//			->doNotRedirect()
//			->setDeleteOldList()
		...
		$this->_modelListConfig = array(
			...
			'mediaDirectory'=>array(
				'fr_media_id'=>'/medias/otc/fr',
				'de_media_id'=>'/medias/otc/de',
				'fr_presentation_media_id'=>'/medias/otc/frPresentation',
				'de_presentation_media_id'=>'/medias/otc/dePresentation',
			),
			'mediaRole'=>array(
				'fr_media_id'=>'user',
				'de_media_id'=>'user',
			...
			'columnLabels'=>array(
				'otc_media_image_id'=>'Image',
				'relation_m2n_otcm2nregion'=>'Region'
			),
			'buttonLabel'=>'Save',
			'columnTypes'=>array(
				'title'=>'text',
	...
	public function updateAction ()
	{
		...
		$backUrl = $this->view->url(array('action'=>'list','controller'=>'otc','module'=>'admin','modelListName'=>'Default_Model_Otc') , NULL, TRUE) . $paramValues;
		$currentUrl = $this->view->url(array('action'=>'update','controller'=>'otc','module'=>'admin','modelListName'=>'Default_Model_Otc') , NULL, TRUE) . $paramValues;

		//redirect to index page if result not found.
		if(!PRJ_PushEmail::getSelectionLists('Default_Model_Otc','OtcM2nDepartment','Otc',$protocolId)){
	...
	public function mailProcessAjaxAction(){
		if ($this->getRequest()->isXmlHttpRequest()) {
			...
			$protocolModel = Default_Model_Otc::getModelByID($protocolId);

			//get all users and email to them
			$result = PRJ_PushEmail::processPushEmail('OtcM2nDepartment', 'Otc', $requestParams, 'otc', $protocolModel->short);
	...
	public function getSelectedUserAjaxAction(){
		if ($this->getRequest()->isXmlHttpRequest()) {
			...
			//get all users and email to them
			$result = PRJ_PushEmail::getUserFromSelection('OtcM2nDepartment','Otc',$requestParams);
	...
	public function sendTestPushmailsAjaxAction ()
	{
		if ($this->getRequest()->isXmlHttpRequest()) {
			...
			//get protocol model.
			$protocolModel = Default_Model_Otc::getModelByID($requestParams['id']);

			//get all users and email to them
			$result = PRJ_PushEmail::sendTestMail($requestParams,'otc', $protocolModel->short);
