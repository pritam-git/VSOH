 ...
 *
 *
 * @filesource /application/modules/admin/controllers/NewsController.php
 * @author     Norbert Marks <nm@l8m.com>
 * @version    $Id: NewsController.php 299 2015-03-31 10:59:38Z nm $
 */
 ...
 *
 */
class Admin_NewsController extends L8M_Controller_Action
{
	/**
	 *
	 *
	 ...
	 *
	 */
	private $_modelListName = 'Default_Model_News';
	private $_modelListShort = 'News';
	private $_modelListConfig = array();
	private $_modelListUntranslatedTitle = 'News';
	...
	/**
	 * Initializes Admin_NewsController.
	 *
	 * @return void
	 */
...
//			->setButton('Update', array('action'=>'update', 'controller'=>'action', 'module'=>'system'), 'update', FALSE)
			->setButton('Send PushMail', array('action'=>'update', 'controller'=>'news', 'module'=>'admin'), 'update', TRUE)
//			->disableSaveWhere()
//			->useDbWhere(FALSE)
//			->showAjax();
//			->doNotRedirect()
//			->setDeleteOldList()
		...
		$this->_modelListConfig = array(
			...
			'mediaDirectory'=>array(
				'fr_media_id'=>'/medias/news/fr',
				'de_media_id'=>'/medias/news/de',
				'fr_presentation_media_id'=>'/medias/news/frPresentation',
				'de_presentation_media_id'=>'/medias/news/dePresentation',
			),
			'mediaRole'=>array(
				'fr_media_id'=>'user',
				'de_media_id'=>'user',
	...
	public function updateAction ()
	{
		...
		$backUrl = $this->view->url(array('action'=>'list','controller'=>'news','module'=>'admin','modelListName'=>'Default_Model_News') , NULL, TRUE) . $paramValues;
		$currentUrl = $this->view->url(array('action'=>'update','controller'=>'news','module'=>'admin','modelListName'=>'Default_Model_News') , NULL, TRUE) . $paramValues;

		//redirect to index page if result not found.
		if(!PRJ_PushEmail::getSelectionLists('Default_Model_News','NewsM2nDepartment','News',$protocolId)){
	...
	public function mailProcessAjaxAction(){
		if ($this->getRequest()->isXmlHttpRequest()) {
			...
			$protocolModel = Default_Model_News::getModelByID($protocolId);

			//get all users and email to them
			$result = PRJ_PushEmail::processPushEmail('NewsM2nDepartment', 'News', $requestParams, 'news', $protocolModel->short);
	...
	public function getSelectedUserAjaxAction(){
		if ($this->getRequest()->isXmlHttpRequest()) {
			...
			//get all users and email to them
			$result = PRJ_PushEmail::getUserFromSelection('NewsM2nDepartment','News',$requestParams);
	...
	public function sendTestPushmailsAjaxAction ()
	{
		if ($this->getRequest()->isXmlHttpRequest()) {
			...
			//get protocol model.
			$protocolModel = Default_Model_News::getModelByID($requestParams['id']);

			//get all users and email to them
			$result = PRJ_PushEmail::sendTestMail($requestParams,'news', $protocolModel->short);
