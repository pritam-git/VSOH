 ...
 *
 *
 * @filesource /application/modules/admin/controllers/RegionDatesController.php
 * @author     Krishna Bhatt <krishna.patel@bcssarl.com>
 * @version    $Id: RegionDatesController.php 299 2019-04-24 10:50:38Z dp $
 */

/**
 *
 *
 * Admin_RegionDatesController
 *
 *
 */
class Admin_RegionDatesController extends L8M_Controller_Action
{
	/**
	 *
	 *
	 ...
	 *
	 */
	private $_modelListName = 'Default_Model_RegionDates';
	private $_modelListShort = 'RegionDates';
	private $_modelListConfig = array();
	private $_modelListUntranslatedTitle = 'RegionDates';
	...
	/**
	 * Initializes Admin_RegionDatesController.
	 *
	 * @return void
	 */
...
//			->setButton('Update', array('action'=>'update', 'controller'=>'action', 'module'=>'system'), 'update', FALSE)
			->setButton('Send PushMail', array('action'=>'update', 'controller'=>'region-dates', 'module'=>'admin'), 'update', TRUE)
//			->setButton('Send PushMail', array('action'=>'send-test-pushmails', 'controller'=>'region-dates', 'module'=>'admin'), 'update', TRUE)
			->setButton('Teilnehmerliste', array('action'=>'download-list', 'controller'=>'region-dates', 'module'=>'admin'), 'download-list', TRUE)
//			->disableSaveWhere()
//			->useDbWhere(FALSE)
//			->showAjax();
//			->doNotRedirect()
//			->setDeleteOldList()
		...
		$this->_modelListConfig = array(
			...
			'mediaDirectory'=>array(
				'fr_media_id'=>'/medias/region-dates/fr',
				'de_media_id'=>'/medias/region-dates/de',
				'fr_presentation_media_id'=>'/medias/region-dates/frPresentation',
				'de_presentation_media_id'=>'/medias/region-dates/dePresentation',
			),
			'mediaRole'=>array(
				'fr_media_id'=>'user',
				'de_media_id'=>'user',
			...
			'columnLabels'=>array(
				'region_dates_media_image_id'=>'Image',
				'relation_m2n_regiondatesm2ncontracttype'=>'Contract',
				'relation_m2n_regiondatesm2nbrand'=>'Brand',
			),
			'buttonLabel'=>'Save',
			'columnTypes'=>array(
				'title'=>'text',
	...
	public function updateAction ()
	{
		...
		$backUrl = $this->view->url(array('action'=>'list','controller'=>'region-dates','module'=>'admin','modelListName'=>'Default_Model_RegionDates') , NULL, TRUE) . $paramValues;
		$currentUrl = $this->view->url(array('action'=>'update','controller'=>'region-dates','module'=>'admin','modelListName'=>'Default_Model_RegionDates') , NULL, TRUE) . $paramValues;

		//redirect to index page if result not found.
		if(!PRJ_PushEmail::getSelectionLists('Default_Model_RegionDates','RegionDatesM2nDepartment','RegionDate',$protocolId)){
	...
	public function mailProcessAjaxAction(){
		if ($this->getRequest()->isXmlHttpRequest()) {
			...
			$protocolModel = Default_Model_RegionDates::getModelByID($protocolId);

			//get all users and email to them
			$result = PRJ_PushEmail::processPushEmail('RegionDatesM2nDepartment', 'RegionDate', $requestParams, 'region-dates', $protocolModel->short, $protocolModel->region_id);
	...
	public function getSelectedUserAjaxAction(){
		if ($this->getRequest()->isXmlHttpRequest()) {
			...
			//get all users and email to them
			$result = PRJ_PushEmail::getUserFromSelection('RegionDatesM2nDepartment','RegionDate',$requestParams);
	...
	public function downloadListAction ()
	{
		...
		$dateModel = Default_Model_RegionDates::getModelByID($dateId);

		//get a new ArrayIterator for sorting the questions with `position` in DB
		$dateQuestions = $dateModel->RegionDatesM2nQuestions->getIterator();
		...
		$userIds = array();
		foreach ($dateModel->RegionDatesParticipants as $participants){
		...
		foreach ($userIds as $userId) {
			...
			$datesParticipantsModel = Doctrine_Query::create()
				...
				->addWhere('region_date_id = ?', array($dateId))
			;
	...
	public function sendTestPushmailsAjaxAction ()
	{
		if ($this->getRequest()->isXmlHttpRequest()) {
			...
			//get protocol model.
			$protocolModel = Default_Model_RegionDates::getModelByID($requestParams['id']);

			//get all users and email to them
			$result = PRJ_PushEmail::sendTestMail($requestParams,'region-dates', $protocolModel->short, $protocolModel->region_id);
